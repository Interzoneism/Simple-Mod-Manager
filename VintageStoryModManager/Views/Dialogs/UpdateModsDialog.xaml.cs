...
public partial class UpdateModsDialog : Window
{
    ...
    private void UpdateModsDialog_OnLoaded(object sender, RoutedEventArgs e)
    {
        if (Owner == null) return;

        ConstrainMaxHeightToOwner();
        Owner.SizeChanged += Owner_OnSizeChanged;
        Closed += (_, _) => Owner.SizeChanged -= Owner_OnSizeChanged;
    }

    private void Owner_OnSizeChanged(object sender, SizeChangedEventArgs e)
    {
        ConstrainMaxHeightToOwner();
    }

    private void ConstrainMaxHeightToOwner()
    {
        if (Owner is not { } owner) return;

        var ownerHeight = owner.ActualHeight-300;
        if (ownerHeight > 0) MaxHeight = ownerHeight;
    }
    ...
    private void CopyChangelogButton_OnClick(object sender, RoutedEventArgs e)
    {
        var changelogText = _viewModel.BuildChangelogClipboardText();
        if (string.IsNullOrWhiteSpace(changelogText)) return;

        try
        {
            WinFormsClipboard.SetDataObject(changelogText, true, 10, 100);
            if (Owner is MainWindow mainWindow)
                mainWindow.ReportStatus("Copied update changelog.");
            else
                StatusLogService.AppendStatus("Copied update changelog.", false);
        }
        catch (ExternalException)
        {
            WpfMessageBox.Show(
                this,
                "Failed to copy the update changelog. Please try again.",
                "Simple VS Manager",
                MessageBoxButton.OK,
                MessageBoxImage.Error);
        }
