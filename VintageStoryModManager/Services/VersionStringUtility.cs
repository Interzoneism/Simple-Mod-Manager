...
internal static class VersionStringUtility
{
    ...
    {
        if (string.IsNullOrWhiteSpace(candidateTag) || string.IsNullOrWhiteSpace(targetVersion))
        {
            return false;
        }

        string trimmedTag = candidateTag.Trim();
        string trimmedTarget = targetVersion.Trim();

        // Exact string match always works
        if (string.Equals(trimmedTag, trimmedTarget, StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }

        // Check if tag is a prefix (e.g., "1.21" matches "1.21.5")
        if (MatchesVersionOrPrefix(trimmedTag, trimmedTarget))
        {
            return true;
        }
        ...
        string? normalizedTag = Normalize(trimmedTag);
        string? normalizedTarget = Normalize(trimmedTarget);

        if (normalizedTag is null || normalizedTarget is null)
        {
            return false;
        }
        ...
        if (tagParts.Length < 2 || targetParts.Length < 2)
        {
            return false;
        }
